...
class MatchDetailsServiceTest {
    ...
    @BeforeEach
    fun setup() {
        com.leinb1dr.pubg.afteractionreport.util.SetupWebClientMock.Builder(webClient).get().uri {
    ...
    fun findMatchById() {
        ...
        Assertions.assertEquals("match", pubgResults.data!![0].type)
        Assertions.assertEquals("bb70dbd7-631d-4d95-8e9e-fc5c2fdcf55a", pubgResults.data!![0].id)
        Assertions.assertTrue(
            OffsetDateTime.now().isAfter((pubgResults.data!![0].attributes as MatchAttributes).createdAt)
        )
    }
    ...
    fun findPlayerParticipant() {
        ...
        Assertions.assertEquals("stealthg0d", find!!.stats.name)
    }
